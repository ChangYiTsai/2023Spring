...
class Ngram:
    ...
    def train_sentiment(self, df_train, df_test):
        ...
        feature_num = 500
        top_features = [pair for pair, count in self.features.most_common(feature_num)]
        ...
        def to_feature_vector(corpus):
            vector = []
            for sentence in corpus:
                sentence_bi_grams = Counter(nltk.ngrams(sentence, 2))
                vector.append([sentence_bi_grams[feature] for feature in top_features])
            return vector
        ...
        train_corpus_embedding = to_feature_vector(train_corpus)
        test_corpus_embedding = to_feature_vector(test_corpus)
    ...
    def save_model(self, path):
        with open(path, 'wb') as f:
            pickle.dump((self.n, self.model, self.features), f)

    def load_model(self, path):
        with open(path, 'rb') as f:
            self.n, self.model, self.features = pickle.load(f)
